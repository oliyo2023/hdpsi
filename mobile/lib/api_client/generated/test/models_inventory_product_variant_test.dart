//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:hd_psi_api/api.dart';
import 'package:test/test.dart';

// tests for ModelsInventoryProductVariant
void main() {
  // final instance = ModelsInventoryProductVariant();

  group('test ModelsInventoryProductVariant', () {
    // 条形码
    // String barcode
    test('to test the property `barcode`', () async {
      // TODO
    });

    // ModelsProductVariantColor color
    test('to test the property `color`', () async {
      // TODO
    });

    // 颜色ID
    // int colorID
    test('to test the property `colorID`', () async {
      // TODO
    });

    // 成本价（可能与主商品不同）
    // double costPrice
    test('to test the property `costPrice`', () async {
      // TODO
    });

    // String createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // ModelsProductVariantFabric fabric
    test('to test the property `fabric`', () async {
      // TODO
    });

    // 面料ID
    // int fabricID
    test('to test the property `fabricID`', () async {
      // TODO
    });

    // int ID
    test('to test the property `ID`', () async {
      // TODO
    });

    // 关联的商品ID
    // int productID
    test('to test the property `productID`', () async {
      // TODO
    });

    // 二维码数据
    // String qRCode
    test('to test the property `qRCode`', () async {
      // TODO
    });

    // 零售价（可能与主商品不同）
    // double retailPrice
    test('to test the property `retailPrice`', () async {
      // TODO
    });

    // 变体SKU
    // String SKU
    test('to test the property `SKU`', () async {
      // TODO
    });

    // ModelsProductVariantSeason season
    test('to test the property `season`', () async {
      // TODO
    });

    // 季节ID
    // int seasonID
    test('to test the property `seasonID`', () async {
      // TODO
    });

    // ModelsProductVariantSize size
    test('to test the property `size`', () async {
      // TODO
    });

    // 尺码ID
    // int sizeID
    test('to test the property `sizeID`', () async {
      // TODO
    });

    // 状态：启用/禁用
    // bool status
    test('to test the property `status`', () async {
      // TODO
    });

    // String updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });


  });

}
